...
check "no reference to the default Ubuntu archive host remains" bash -c \
  "! grep -qrE 'https?://archive\.ubuntu\.com' /etc/apt/sources.list /etc/apt/sources.list.d/*.sources 2>/dev/null"
check "security.ubuntu.com is left untouched" bash -c \
  "grep -qrE 'https?://security\.ubuntu\.com' /etc/apt/sources.list /etc/apt/sources.list.d/*.sources 2>/dev/null"
